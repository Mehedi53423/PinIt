...
function createWithPortableTextLists(portableTextFeatures, change$) {
  return function withPortableTextLists(editor) {
    ...
    editor.pteUnsetListItem = listItemStyle => {
      ...
      change$.next({
        type: 'selection',
        selection: (0, _ranges.toPortableTextRange)(editor)
      });
      editor.onChange();
    };
    ...
    editor.pteEndList = () => {
      ...
      var selectedBlocks = [..._slate.Editor.nodes(editor, {
        at: editor.selection,
        match: node => _slate.Element.isElement(node) && node._type === portableTextFeatures.types.block.name && !!node.listItem && node.children.length === 1 && node.children[0].text === ''
      })];
    ...
    editor.pteIncrementBlockLevels = reverse => {
      ...
      var selectedBlocks = [..._slate.Editor.nodes(editor, {
        at: editor.selection,
        match: node => !!(_slate.Element.isElement(node) && node.listItem)
      })];
      ...
      selectedBlocks.forEach(_ref7 => {
        ...
        var level = typeof node.level === 'number' ? node.level : 1;
    ...
    editor.pteHasListStyle = listStyle => {
      ...
      var selectedBlocks = [..._slate.Editor.nodes(editor, {
        at: editor.selection,
        match: node => _slate.Element.isElement(node) && node.listItem === listStyle
      })];
