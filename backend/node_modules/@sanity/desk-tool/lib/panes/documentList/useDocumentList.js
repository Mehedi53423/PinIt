...
function useDocumentList(opts) {
  ...
  var query = (0, _react.useMemo)(() => {
    ...
    if (extendedProjection) {
      ...
      return ["*[".concat(filter, "] [0...").concat(limit, "]"), "{".concat(firstProjection, "}"), "order(".concat(order, ")"), "{".concat(finalProjection, "}")].join('|');
    }
  ...
  }, [fullList, query, params]); // If `filter` or `params` changed, set up a new query from scratch.
